...
export function previewCommand(text) {
  ...
  if (cmd === "/google") {
    ...
    // passiert" aus.
    showPanel('<div class="cmd-hint">Weiter tippen: <strong>login</strong> · <strong>sync</strong> · <strong>trennen</strong> — oder Enter für den Status.</div>');
    return;
  }
  if (cmd === "/k") {
    const my = ++kToken;
...
export async function runCommand(text) {
  ...
  switch (cmd) {
    ...
    case "/google": {
      ...
      try {
        if (a === "login" || a === "verbinden" || a === "an" || a === "connect") {
          ...
          flash(s.connected
            ...
            : "Google nicht verbunden — „/google login“ ausführen.", s.connected ? "ok" : "warn");
        }
      } catch (err) {
